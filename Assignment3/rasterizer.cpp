...
void rst::rasterizer::rasterize_triangle(const Triangle& t, const std::array<Eigen::Vector3f, 3>& view_pos) 
{
    ...
    //SSAA verison
    auto v = t.toVector4();
    float Xmin = std::max(0.0f,std::min(v[0].x(), std::min(v[1].x(), v[2].x())))*ssaa_factor;
    float Xmax = std::min(float(width-1),std::max(v[0].x(), std::max(v[1].x(), v[2].x())))*ssaa_factor;
    float Ymin = std::max(0.0f,std::min(v[0].y(), std::min(v[1].y(), v[2].y())))*ssaa_factor;
    float Ymax = std::min(float( height - 1), std::max(v[0].y(), std::max(v[1].y(), v[2].y())))*ssaa_factor;
    for (int y = floor(Ymin); y < ceil(Ymax); y++)
    {
        for (int x = floor(Xmin); x < ceil(Xmax); x++)
        {
            ...
            if (insideTriangle(pixel_x, pixel_y, t.v))
            {
                ...
                zp *= Z;
                int ssaa_index = get_ssaa_index(x, y);
                if (zp < depth_buf_ssaa[ssaa_index])
                {
                    //进行插值运算
                    ...
                    depth_buf_ssaa[ssaa_index] = zp;
                    frame_buf_ssaa[ssaa_index] = pixel_color;
                }
            }
        }
    }
...
rst::rasterizer::rasterizer(int w, int h) : width(w), height(h)
{
    ...
    frame_buf_ssaa.resize(w * h*ssaa_factor*ssaa_factor);
    depth_buf_ssaa.resize(w * h*ssaa_factor*ssaa_factor);
    texture = std::nullopt;
}
    ...
    for (int y = 0; y < height; y++)
    {
        for (int x = 0; x < width; x++)
        {
            ...
            for (int dy = 0; dy < ssaa_factor; dy++)
            {
                ...
                {
                    int ssaa_index = get_ssaa_index(x*ssaa_factor  + dx, y*ssaa_factor + dy);
                    avg_color += frame_buf_ssaa[ssaa_index];
                }
            }
